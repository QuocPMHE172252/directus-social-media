...
export function ReactionBar({ postId, compact = false }: { postId: string; compact?: boolean }) {
	...
	async function refresh() {
		try {
			...
			if (res.ok) {
				...
				// Hỗ trợ cả 2 dạng: { summary, hasLiked } hoặc summary thuần
	...
	async function react(type: string) {
		...
		try {
			...
			if (response.ok) {
				...
				setLiked(wasLiked);
				setLastAction(`❌ Please login to like posts`);
				setTimeout(() => { window.location.href = '/login'; }, 1200);
			} else {
				// revert
				setSummary(prev => ({ ...prev, [type]: Math.max((prev[type] ?? 0) + (wasLiked ? 1 : -1), 0) }));
				setLiked(wasLiked);
				setLastAction(`❌ Failed: ${(data && (data.message || data.error)) || 'Unknown error'}`);
			}
		} catch (error) {
			// revert
	...
	if (compact) {
		return (
			<div className="flex items-center justify-center gap-2">
				...
				>
					<svg className={`w-5 h-5 ${likedColor}`} fill="currentColor" viewBox="0 0 20 20">
	...
	return (
		<div className="flex items-center gap-2 text-sm">
			...
			{lastAction && (
				<span className={`text-xs ${lastAction.includes('✅') ? 'text-green-600' : 'text-red-600'}`}>
					{lastAction}
				</span>
			)}
		</div>
